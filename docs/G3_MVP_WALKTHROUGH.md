...
| A-c | 打开设置完整页（5 卡版） | 页面底材质观感：玻璃折射成立或你认可现状（F5 撤底后） | D-10/D-12 | ☐ **⁽⁰⁹⁻⁰⁷ᶠ⁾ F5 已随 D-10 改判撤销**（卡底回到不透明 `.surface`；F5 唯一立论＝透明窗底之下挡 sheet 透出，立论失效）⇒ 本项改判「概览五卡与 sheet 的层次/可读性」，折射议题随 (c) 关闭 |
| A-d | 观察聊天区/输入框玻璃质感 | **F4(a) 已实施@`ee1da43`：判定侧栏/窗体是否真实采到桌面折射（或接受扁平→可回退）** | D-10 已拍(a) | ❌ **⁽⁰⁹⁻⁰⁷ᶠ⁾ 用户否决 (a)**（侧栏浓橙＝玻璃采用户暖棕壁纸，色相随壁纸漂移）⇒ **(a) 已回退、改判 (c) 接受扁平**。**复验口径**：侧栏应为**中性浅灰且不随壁纸变色**（探针实测修复态玻璃面＝230,230,230），观感不满意则整条留在 (c) 不再回 (a)。**若仍判坏＝下一条预置回退（已与本条分开）**：撤除侧栏自身两块全屏玻璃面（`SidebarView.swift` 展开面 `.glassSurface(.regular, morphID:)` ＋折叠面 `.prominent`），代价＝折叠/展开 morph 退化为交叉淡变（P1.3 功能不损） |
