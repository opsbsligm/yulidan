...
| # | 走查动作（你手动） | 判定标准 | 依赖 | 状态 |
|---|--------------------|----------|------|------|
| A-a | 打开主窗，观察 Tab 条切换选中 | 选中玻璃面流体融合/形变过渡（非淡变方块）→ D-1 morph 终判 | D-1 | ✅ **⁽⁰⁹⁻⁰⁷ᵍ⁾ 09-07 13:27 复验通过 ⇒ 回退方案定稿**（新包 pid 97889，三页目检无融片）；历史：⁽⁰⁹⁻⁰⁷ᵉ⁾ 首次目检＝❌ 不通过（用户实机截图）⇒ 铺满方案已回退：六段常驻面＋容器 spacing 52 让网格融成一整片橙色玻璃；代码已恢复「仅选中段带面／spacing=fullGridSpacing」，护栏方向反转为上限（native＝1 面）。**本项需你在下次 relaunch 后复验回退效果**；morph 流体融合本身仍未证实（第二面缺口 §3-A1 回到在册未决，不用铺满去填）。⁽⁰⁹⁻⁰⁷ᵃ⁾「前提已消除」的说法**已被本轮目检推翻**：：09-06 批次已把常驻玻璃面铺满全部分段（护栏 `glassFaceCount == segmentCount`，native 态）⇒ morph 现有第二面可配对，**本项自 09-07 起为「对已实施效果的终裁」而非「先拍方案再实施」**；⚠️ 但 A 层无可捕获玻璃像素的通道，观感结论只能出自本项现场目检 |
| A-b | 折叠/展开侧栏各 1 次（⁽⁰⁹⁻⁰⁷ᵍ⁾ 09-07 三张截图均为**展开态**⇒ 折叠 rail 未入镜，本项未做） | 折叠 rail 左上角与红绿灯/展开按钮无交叠压迫感（F6 修后）或你接受现状 | D-12(F6) | ✅ **⁽⁰⁹⁻⁰⁷ʰ⁾ 09-07 14:2x 用户口令核销**（chat 原文『1全部核销』）＝**笼统放行，非逐项目检**；单条 revert 即回退口径不变。 原判定项在册保留（F6 折叠态下沉 38pt＋按钮右移；交叠判据 before/after 双锁已在测试内）（`SidebarRailLayout`：折叠态顶部下沉 38pt＋按钮右移至 rail 之右；交叠判据 before/after 双锁）⇒ 本项判「实施后的实际观感」，含一项在册偏差待你裁：**按钮必须同时右移**，否则与下沉后的 rail 首图标重新交叠 |
| A-c | 打开设置完整页（5 卡版） | 页面底材质观感：玻璃折射成立或你认可现状（F5 撤底后） | D-10/D-12 | ☐ **⁽⁰⁹⁻⁰⁷ᶠ⁾ F5 已随 D-10 改判撤销**（卡底回到不透明 `.surface`；F5 唯一立论＝透明窗底之下挡 sheet 透出，立论失效）⇒ 本项改判「概览五卡与 sheet 的层次/可读性」，折射议题随 (c) 关闭。**⁽⁰⁹⁻⁰⁷ᵍ⁾ 旁证（不核销）**：用户 09-07 截图③＝设置 sheet「偏好」页，卡片边界/分组清晰（F5 撤销后可见恢复）；**但 5 卡概览页本身仍未走过**，本项保持待你判 → ✅ **⁽⁰⁹⁻⁰⁷ʰ⁾ 09-07 14:2x 用户口令核销**（chat 原文『1全部核销』）＝**笼统放行，非逐项目检**；单条 revert 即回退口径不变。 |
| A-d | 观察聊天区/输入框玻璃质感 | **F4(a) 已实施@`ee1da43`：判定侧栏/窗体是否真实采到桌面折射（或接受扁平→可回退）** | D-10 已拍(a) | ✅ **⁽⁰⁹⁻⁰⁷ᵍ⁾ 09-07 13:27 复验通过 ⇒ D-10 改判 (c) 定稿**。现算像素（用户三张截图，同一侧栏区域）＝(252,252,252)／(242,243,249)／(240,243,249) ⇒ **R−B = 0／−7／−9（中性）**，回退前同一区域 **+54**（浓橙）⇒ **界面不再随暖棕壁纸染色**，与探针预测 (230,230,230) 同向。历史：⁽⁰⁹⁻⁰⁷ᶠ⁾ 用户否决 (a)（侧栏浓橙＝玻璃采用户暖棕壁纸）。**复验口径**：侧栏应为**中性浅灰且不随壁纸变色**（探针实测修复态玻璃面＝230,230,230），观感不满意则整条留在 (c) 不再回 (a)。**若仍判坏＝下一条预置回退（已与本条分开）**：撤除侧栏自身两块全屏玻璃面（`SidebarView.swift` 展开面 `.glassSurface(.regular, morphID:)` ＋折叠面 `.prominent`），代价＝折叠/展开 morph 退化为交叉淡变（P1.3 功能不损） |
| A-e | 主题插件切 tint → 还原 | 实时染色生效、还原干净；作用域观感符合你对 D-11 的选择 | D-11 | ☐ |
| A-f | 会话拖拽换序一次 | 落位原生反馈动画自然（R1⑦ 终判） | — | ☐ |
| A-g | 系统设置开「降低透明度」→ 回开 | 界面即时降级/恢复无残影（R1⑧ 终判） | — | ☐ |
| A-h | 减弱动效开关往返 | morph/入场动画按系统开关注销 | — | ✅ **⁽⁰⁹⁻⁰⁷ʰ⁾ 09-07 14:2x 用户口令核销**（chat 原文『1全部核销』）＝**笼统放行，非逐项目检**；单条 revert 即回退口径不变。（降级链本体有 A 层证据在册＝R1#8/`GlassSurfaceTests`） |

> **⁽⁰⁹⁻⁰⁷ʰ⁾ 池 A 结论（09-07 14:2x）**：A-a/A-d＝**逐项目检通过**（附现算像素）；A-b/A-c/A-h＝**用户口令『1全部核销』笼统放行**。
> ⇒ **池 A 全部关闭**，但两种核销性质不同，账面分列不混记；被口令放行的三项若日后你看着不顺，各自仍是单条小回退（F6＝`SidebarRailLayout` clearance 常量／F5＝卡底 `.surface`／减弱动效＝`GlassSurfaceModifier` 降级链）。
...
| # | 动作 | 判据 | 依赖 | 状态 |
|---|------|------|------|------|
| C-a | 轴2 取证（二选一）：你从 Codex 各观察窗丢截图 / 你择时允许只读截自己另开的观察窗 | BENCHMARK §16 W1-W8 取证齐 → UI_CODEX_ALIGNMENT 行为列回正 ⁽⁰⁹⁻⁰⁶ᵉ⁾ 截图通道已实测＝锁屏态纯 A 层可用（含内容活性自证）| 取证方式拍板 | ✅ **⁽⁰⁹⁻⁰⁷ʰ⁾ 用户口令放行（『1全部核销』）＝W2–W8 不再取证**。**诚实边界（不得美化）**：判据原文要『取证齐』，本项实际＝**W1 自主闭环（四件套在册）＋W2–W8 未取证按现状放行**；`UI_CODEX_ALIGNMENT` 的 W2–W8 行为列保持『未取证』标注，**不得回正为已对标**。|
| C-b | G1a 轴1 清单终查 | BENCHMARK_CHECKLIST 全项 ✅ 或 N/A 有论证 | — | ✅ **⁽⁰⁹⁻⁰⁷ʰ⁾ 用户口令核销**（依据＝09-04 Agent 预检 ☐=0 全闭合／N-A 有论证，现算复跑仍 ☐=0） |
...
| D-10 | F4 折射源修法 | (a) 窗口透明底 / (b) backgroundExtensionEffect / (c) 接受扁平 | ✅ 09-04 = (a) @`ee1da43` → **⁽⁰⁹⁻⁰⁷ᶠ⁾ 改判 (c) 接受扁平**（A-d 目检否决 ⇒ 已整条回退） |
...
| **RSS 可见态组** | 可见态性能基线 | 见总账 `DECISION_INDEX` 对应行 | ✅ **⁽⁰⁹⁻⁰⁷ʰ⁾ 用户口令认可 5 点数据为可见态基线（正式平台期判定不再追加）** ｜✅ 09-06 = ✅ 09-06 **要基线**；随 G3 走查同场后台只读采集，不单独占用你的时间。**⁽⁰⁹⁻⁰⁷ᵍ⁾ 已采完（你未出场）**：可见态实例 pid 97889（你 13:27 手 relaunch 的新包，`verify applyOpaqueBackdrop` 符号对拍在位），观察窗 14:03:20→14:15:21＝12 分 01 秒，5 点 94.7/82.1/80.3/88.6/**79.6 MB**（漂移 −15.9%，pcpu 恒 0）⇒ **G3 手册口径（自然使用 ≥10 分钟）满足**；⚠️ 09-03 提案的「点间隔 ≥10 分钟」未满足 ⇒ PERFORMANCE 记为「趋势平稳·平台期未正式判定」，补正式判定只需我静默再采 2 点（间隔 ≥10 分钟），不用你出场 |
