...
|---|---|---|
| `glassEffect(_:in:)` | "Renders a shape anchored behind a view with the Liquid Glass material. Applies the foreground effects of Liquid Glass over a view."；默认 `.regular` + `DefaultGlassEffectShape`；"SwiftUI anchors the Liquid Glass to a view's bounds."＋"For the example above, the material fills the entirety of the frame, which includes the padding."；"You typically use this modifier with a [GlassEffectContainer] to combine multiple Liquid Glass shapes into a single shape that can morph into one another."〔09-05 逐字复核更正：原摘录系改写，官方原句见 BENCHMARK §24.4〕（且原句②把官方 `GlassEffectContainer` 错填成 `[glassEffectID]`，属占位还原错误，见 §24.4） | 玻璃锚定 view bounds（改 frame 即改玻璃）；morph 必须配 glassEffectID |
| `glassEffectUnion(id:namespace:)` | "You may want the geometries of multiple views to contribute to a single Liquid Glass effect shape."＋"All Liquid Glass effects with the same shape and Liquid Glass variant will be combined into a single shape"〔09-05 逐字复核：前半原系改写语序，官方为 "You may want the geometries of multiple views to…"；后半在线页与 26.5/27 `.swiftdoc` 措辞已漂移（SDK 侧作 "All glass effects with the same shape and glass…"），漂移实例见 §24.5〕 | 并集融合要求同 shape + 同材质变体——**Tab 分段与 morph 目标面必须用同一 Glass 变体与同型 shape** |
| `glassEffectID(_:in:)` | "You use this modifier with the [glassEffect(_:in:)] view modifier and a [GlassEffectContainer] view. When used together, SwiftUI uses the identifier to animate shapes to and from each other during transitions"〔09-05 逐字复核：两个占位此前均还原错——官方正文是 `glassEffect(_:in:)`＋`GlassEffectContainer`，我方写成 `[glassEffect]`＋`[Namespace]`；`.swiftdoc` 同句作 "SwiftUI will use the provided identifier"，与在线页并存，属 §24.5 漂移〕 | 官方确认 = 目标 P1 §2 的 morph 机制（非模拟） |
