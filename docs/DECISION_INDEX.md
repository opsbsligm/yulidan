...
| **D-6** | 24 零事件会话处置 | 删（DB 写需明示+二次确认）/ 留 | ✅ 09-04 = 删（DELETE 待二次确认执行） | QUALITY DB 只读复核（26=24+2）；Keychain 旧凭证为可选项 |
