...
## 七、质量基线（已实测，供核对）
- **当前基线（2026-08-25 周末覆盖审计轮 11，四门禁全绿 @2b5744d）**：pr 门禁 954/954（Swift Testing 732/732 157 suites + XCTest 222/222，/tmp/ci_pr_agent11.log）+ main 门禁（Release + 全量 + 覆盖率：核心 14 包 97.50%（6940/7118）均 ≥90% / 18 包全量 95.38%（9505/9965）/ Subagent 95.85%（16 未覆盖行全定性）；干净单跑核验核心 14 包 97.57%（6945/7118）/ 18 包 95.43%（9510/9965），/tmp/ci_main_agent11.log）+ leaks 门禁（MemProbe 500 → **0 leaks**，/tmp/ci_leaks_agent11.log）+ xcode 门禁（**TEST SUCCEEDED**，/tmp/ci_xcode_agent11.log）；前轮基线：952/952（覆盖审计轮 10 docs-only 复跑 @2dcbc65 /tmp/ci_pr_agent10.log）/ 952/952（覆盖审计轮 9 /tmp/ci_pr_agent9.log）/ 946/946（覆盖审计轮 8 /tmp/ci_pr_agent8c.log）/ 940/940（覆盖审计轮 7 /tmp/ci_pr_agent7.log）
