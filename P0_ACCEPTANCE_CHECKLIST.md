...
## 七、质量基线（已实测，供核对）
- **当前基线（2026-08-26 周末覆盖审计轮 14，四门禁全绿 @54fa2d3）**：pr 门禁 961/961（Swift Testing 739/739 160 suites + XCTest 222/222，/tmp/ci_pr_agent14.log）+ main 门禁（Release + 全量 + 覆盖率，干净口径：核心 14 包 97.85%（6965/7118）均 ≥90% / 18 包全量 95.63%（9530/9965）/ Tools 98.43%（10 未覆盖行全定性），/tmp/ci_main_agent14.log）+ leaks 门禁（MemProbe 500 → **0 leaks**，/tmp/ci_leaks_agent14.log）+ xcode 门禁（**TEST SUCCEEDED** 7 bundle，/tmp/ci_xcode_agent14.log）；前轮基线：960/960（覆盖审计轮 13 /tmp/ci_pr_agent13.log）/ 957/957（覆盖审计轮 12 /tmp/ci_pr_agent12.log）/ 954/954（覆盖审计轮 11 /tmp/ci_pr_agent11.log）/ 952/952（覆盖审计轮 10 docs-only 复跑 @2dcbc65 /tmp/ci_pr_agent10.log）/ 952/952（覆盖审计轮 9 /tmp/ci_pr_agent9.log）
