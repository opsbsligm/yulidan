...
> ✅ **2026-08-27 P0 补充②：「上下文大小」v1 假配置实锤 → Ollama 原生 API 重构（实证 + 修复闭环，四门禁全绿）**：🔴 **15:34 实机实证（本轮核心发现）**：Ollama OpenAI 兼容端点 /v1/chat/completions **不受理** `options` 字段——curl 下发 `{"options":{"num_ctx":32768}}` → 200，但 Ollama 为此请求新起的 llama-server（启动时刻与请求吻合）参数仍是 **`-c 4096`**（进程参数 = 上下文生效 ground truth）；官方兼容端点文档字段清单（max_tokens/tools/reasoning_effort/stream_options 等）**无 options** → v1 的上下文大小对本地模型是**假配置**（服务端静默忽略，UI 无任何错误提示，属最危险的一类缺口）。**修复**：① 新增 `OllamaNativeChat`（原生 /api/chat：options.num_ctx/num_predict + 顶层 think + NDJSON 流式 + message.thinking→reasoning + done_reason 映射 + prompt_eval_count/eval_count→usage；原生 tool_calls 无 id → 合成 call-i；**官方 tool 结果消息须带 `tool_name` 字段——官方文档在案，本轮复核发现 v1 wire 缺失并补齐**）；② LocalAdapter 引擎探测：GET {nativeBase}/api/tags 200 且含 models key → ollamaNative，否则/失败 → openAICompat（实例缓存 + /v1 后缀剥离；vLLM/LM Studio 保持兼容路径且**不下发** numCtx）；③ 回滚 OpenAI 兼容分支的 numCtx/options 死字段（不留假配置入口）；④ UI 两处文案改诚实（上下文大小 = 仅 Ollama 本地模型生效、原生 API options.num_ctx 真实下发；思考等级 = 下发 Ollama）。**测试**：删 v1 失效测试 2 + 新增 12（OllamaNativeChatTests 7：请求 wire options/think/stream:false、无死字段、工具历史 wire（arguments=JSON 对象 + tool_name）、非流式解析（thinking→reasoning / toolCalls JSON 比较 / usage 10,5,15 / finishReason）、done_reason 映射 8 例、NDJSON 事件序列 text→reasoning→text→done、500→httpError；LocalAdapterEngineRoutingTests 5：探测命中 → /api/chat 且 num_ctx=131072、tags 404 → 兼容且无 options、tags 599 → 兼容兜底、tags 200 但无 models key → 兼容、流式原生路由 + message_complete 终态）。**四门禁全绿**：PR（SwiftLint/SwiftFormat 0 违规 / 构建 0 警告 / XCTest 255/255 + Swift Testing 810/810）+ **leaks 0** + **main**（release 构建 + 覆盖率：后端 Sources 90 文件 10,557 行 **96.13%**，较 v1 轮 95.58% 提升 0.55pp）+ **xcode BUILD SUCCEEDED / TEST SUCCEEDED**。⏳ **唯一剩余项 = 实机铁证**：设置上下文大小=32K → 发一条消息 → `pgrep -f llama-server | xargs ps -o command=` 须见 **`-c 32768`**（num_ctx 真实生效的唯一 ground truth）+ 设置页新文案截图（18:15 机器锁屏中，待解锁窗口执行；App 已更新新 bundle sha 5320b066，75 个 OllamaNativeChat 符号 nm 核验在位）。
