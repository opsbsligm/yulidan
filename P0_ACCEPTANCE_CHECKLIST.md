...
## 七、质量基线（已实测，供核对）
- **当前基线（2026-08-26 周末覆盖审计轮 15，四门禁全绿 @ef05356）**：pr 门禁 966/966（Swift Testing 743/743 161 suites + XCTest 223/223，/tmp/ci_pr_agent15.log）+ main 门禁（Release + 全量 + 覆盖率，干净口径：核心 14 包 97.91%（6969/7118）均 ≥90% / 18 包全量 95.67%（9534/9965）/ Workspace 98.58%（4 未覆盖行全 thunk）/ MCP 96.25%（30 未覆盖行全定性），/tmp/ci_main_agent15.log）+ leaks 门禁（MemProbe 500 → **0 leaks**，/tmp/ci_leaks_agent15.log）+ xcode 门禁（**TEST SUCCEEDED** 7 bundle，/tmp/ci_xcode_agent15.log）；前轮基线：961/961（覆盖审计轮 14 /tmp/ci_pr_agent14.log）/ 960/960（覆盖审计轮 13 /tmp/ci_pr_agent13.log）/ 957/957（覆盖审计轮 12 /tmp/ci_pr_agent12.log）/ 954/954（覆盖审计轮 11 /tmp/ci_pr_agent11.log）/ 952/952（覆盖审计轮 10 docs-only 复跑 @2dcbc65 /tmp/ci_pr_agent10.log）
