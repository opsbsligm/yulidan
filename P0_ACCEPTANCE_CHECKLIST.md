...
> ✅ **2026-08-27 P0 补充：「上下文大小」设置字段（第 4 批报障第 2 项唯一残留缺口，代码 + wire 单测证据闭环）**：设置 → 请求参数新增「上下文大小」行——直输 + 预设 8K/16K/32K/64K/128K/256K/1M（范围 1,024–1,048,576，越界不保存，留空 = 跟随模型默认）；**仅本地提供商生效** → 经 Ollama `options.num_ctx` 下发（wire 格式经 URLProtocol 桩断言 `"options":{"num_ctx":N}`；远程提供商不下发，UI 已诚实标注），AppViewModel → AgentLoop.setTurnContext → LLMRequest → Adapters → OpenAICompatChat 全链路传播单测（Agent 传播 2 例 + LLM wire 2 例 + 配置归一化 1 例 + legacy 解码兼容，全绿）。四门禁全绿：**PR**（SwiftLint/SwiftFormat 0 违规 / 构建 0 警告 / 810/810，基线 808 + 2 net new，stash 对照）+ **leaks 0** + **main**（release 构建 + 覆盖率：后端 17 包行覆盖 95.58%，LLMConfigStore 86.32% 新增逻辑全覆盖；SettingsSubPages 0% 属 UI 层惯例=截图核验覆盖）+ **xcode TEST SUCCEEDED**。⏳ 实机截图核验待用户机器空闲窗口补拍（13:3x 用户 WeChat 作业中不抢焦点；App 已重启至新 bundle PID 18256 @sha 86f55bc，用户可自点「设置 → 请求参数」直接查看）。

> ✅ **2026-08-27 P0 补充②：「上下文大小」v1 假配置实锤 → Ollama 原生 API 重构（实证 + 修复闭环，四门禁全绿）**：🔴 **15:34 实机实证（本轮核心发现）**：Ollama OpenAI 兼容端点 /v1/chat/completions **不受理** `options` 字段——curl 下发 `{"options":{"num_ctx":32768}}` → 200，但 Ollama 为此请求新起的 llama-server（启动时刻与请求吻合）参数仍是 **`-c 4096`**（进程参数 = 上下文生效 ground truth）；官方兼容端点文档字段清单（max_tokens/tools/reasoning_effort/stream_options 等）**无 options** → v1 的上下文大小对本地模型是**假配置**（服务端静默忽略，UI 无任何错误提示，属最危险的一类缺口）。**修复**：① 新增 `OllamaNativeChat`（原生 /api/chat：options.num_ctx/num_predict + 顶层 think + NDJSON 流式 + message.thinking→reasoning + done_reason 映射 + prompt_eval_count/eval_count→usage；原生 tool_calls 无 id → 合成 call-i；**官方 tool 结果消息须带 `tool_name` 字段——官方文档在案，本轮复核发现 v1 wire 缺失并补齐**）；② LocalAdapter 引擎探测：GET {nativeBase}/api/tags 200 且含 models key → ollamaNative，否则/失败 → openAICompat（实例缓存 + /v1 后缀剥离；vLLM/LM Studio 保持兼容路径且**不下发** numCtx）；③ 回滚 OpenAI 兼容分支的 numCtx/options 死字段（不留假配置入口）；④ UI 两处文案改诚实（上下文大小 = 仅 Ollama 本地模型生效、原生 API options.num_ctx 真实下发；思考等级 = 下发 Ollama）。**测试**：删 v1 失效测试 2 + 新增 12（OllamaNativeChatTests 7：请求 wire options/think/stream:false、无死字段、工具历史 wire（arguments=JSON 对象 + tool_name）、非流式解析（thinking→reasoning / toolCalls JSON 比较 / usage 10,5,15 / finishReason）、done_reason 映射 8 例、NDJSON 事件序列 text→reasoning→text→done、500→httpError；LocalAdapterEngineRoutingTests 5：探测命中 → /api/chat 且 num_ctx=131072、tags 404 → 兼容且无 options、tags 599 → 兼容兜底、tags 200 但无 models key → 兼容、流式原生路由 + message_complete 终态）。**四门禁全绿**：PR（SwiftLint/SwiftFormat 0 违规 / 构建 0 警告 / XCTest 255/255 + Swift Testing 810/810）+ **leaks 0** + **main**（release 构建 + 覆盖率：后端 Sources 90 文件 10,557 行 **96.13%**，较 v1 轮 95.58% 提升 0.55pp）+ **xcode BUILD SUCCEEDED / TEST SUCCEEDED**。✅ **服务端铁证（2026-08-27 18:49，锁屏窗口 curl 直验，无 UI 依赖）**：curl 原生 /api/chat 下发 `options.num_ctx=32768, num_predict=64` → 200（done_reason=length，qwen3:4b 思考模型 64 token 全耗 thinking）→ Ollama 新起 llama-server 实例（PID 85813，18:49:52 启动时刻与请求吻合）参数 **`-c 32768`**（进程参数 = 上下文生效 ground truth，证据 /tmp/llama_evidence_c32768.txt）——**与 15:34 兼容端点实证（`-c 4096`）形成对照闭环**：服务端侧「原生受理 / 兼容忽略」双向实锤。链路证明 = 单测锁死我方 wire（LocalAdapter 路由 → /api/chat + options.num_ctx 在体）× 服务端实证（num_ctx → llama-server -c）⇒ 全链路成立。✅ **生产代码路径端到端铁证（19:04，opt-in 集成测试 `HARNESS_INTEGRATION=1 swift test --filter OllamaNativeIntegrationTests`）**：`LocalAdapter`（/api/tags 探测→原生路由）→ `OllamaNativeChat`（options.num_ctx=32768）→ 真实 Ollama → `/api/ps` 报告 **`context_length: 32768`**（官方字段，与进程参数 `-c 32768` 同义）——**非 curl，走的即 App 同一份 Swift 代码**；无环境变量时整类 XCTSkip，门禁零依赖。✅ **2026-08-28 11:55 配置持久化级端到端铁证（用户要求「其他方法验证」→ 新增 opt-in 集成测试，全程零 UI 干扰）**：`HARNESS_INTEGRATION=1 swift test --filter ConfigPersistenceIntegrationTests`（Apps/HarnessApp/Tests/ConfigPersistenceIntegrationTests.swift，首跑 4.6s 通过）——UI 保存契约（LLMConfig.save() = 设置页「32K」预设按钮同款路径：normalized 归一化 + JSON→UserDefaults）→ LLMConfig.load() App 同源往返（contextWindow=32768 保留断言）→ App 同款 LocalAdapter 构造（AppViewModel .local 分支语义：ProviderProfile.local(forModel:)）→ 真实 Ollama → /api/ps **context_length: 32768**（官方字段）。无环境变量整类 XCTSkip，门禁零依赖。**证据链升级至四层**：① 桩单测 wire×12 ② curl 服务端 `-c 32768` 进程参数 ③ 生产路径集成测试（LocalAdapter 起步，19:04）④ 配置持久化集成测试（UI 保存契约起步，11:55）。⏳ **剩余 = 像素级点击自查（用户清单项 ④，约 10 秒）**：设置→请求参数→点 32K→保存→发一条消息（新 bundle 直接打开即当前构建）。备注：App（bundle sha 5320b066，PID 78235）浸泡证据 08-27 18:12 启动 → 08-28 上午用户手动退出（10:11 复测 etime 15h58m 零 crash 在案，DiagnosticReports 全程零记录；11:09 曾后台重启实例 85769 用于 AX 走查，约 1 分钟后被用户关闭，无 crash 无副作用）。
...
## 七、质量基线（已实测，供核对）
- **当前基线（2026-08-27 Ollama 原生 API 重构轮，四门禁全绿 @97de3eb）**：pr 门禁（SwiftLint strict 0 违规 / SwiftFormat 0/254 文件 / 构建 0 警告 / **XCTest 257（2 skip = 两个 opt-in 集成测试）+ Swift Testing 810/810（175 suites）合计 1067，0 失败**，/tmp/ci_pr_cfgchain_full.log @本轮配置持久化铁证轮）+ leaks 门禁（**0 leaks**，/tmp/ci_leaks_native.log）+ main 门禁（Release + 全量 + 覆盖率：**后端 Sources 90 文件 10,557 行 96.13%**（前轮 95.58% +0.55pp；<90% 仅 4 文件，AppleSignInService 15.86% 属 entitlements 依赖不可单测区已标注），/tmp/ci_main_native.log）+ xcode 门禁（**BUILD SUCCEEDED / TEST SUCCEEDED**，/tmp/ci_xcode_native.log）+ opt-in 实机集成铁证（`HARNESS_INTEGRATION=1`：/api/ps context_length=32768，3.1s 通过）；**App 浸泡：新 bundle sha 5320b066 16h+ 零崩溃**（08-27 18:12 启动，08-28 上午用户手动退出；19:18 / 08-28 10:11 两次 DiagnosticReports 均零 crash 记录，10:11 被动复测 PID 78235 etime 15:58:08 连续运行、Ollama /api/ps 零 runner 残留）；前轮基线：四门禁 @0991eab（2026-08-26 轮 18）
