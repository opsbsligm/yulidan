...
#### §10.3-A 静默口径核销材料（09-06 逐项实测复核，@HEAD a53adac）

> 用途：v8 §G0 要求 #3/#6/#7/#8「依 A 层在册证据 + 你的二选一裁决」。下表把每项的
> **在册锚点重新实测过一遍**（今日已查出 4 类文档失真，故不采信记忆），并明说
> **A 层能证到哪一步**——你只需对每项回「认可 A 层」或「我 1 分钟配合」。
...
"`:96` 溢出 `Menu { sessionMenuActions }`／`:133` 唯一定义——四处逐行实测命中；"
"#6 锚点 `ThemeLiveRenderTests.swift:85`（@Test 行号精确，`:102` 切 sunset／`:112` 还原基准）命中；"
"#8 锚点 `GlassSurfaceTests.swift:191`（`currentMode(envReduceTransparency: true) == .solid`，"
"前置断言 override==nil）＋ `:22`/`:29` 测试缝双向 + `@Test` 计数 19——命中。**⇒ 本表四类锚点现全部"
"经 09-06 实跑/实搜复核，唯一失真＝#7，已就地改判并留档。**"
"另按㉘口径附带核验：`swift test --filter` 传中文 display name 会 0 命中，"
"此前个别「未跑」表述若与本表冲突，一律以本表实跑条数为准。"
...
| #3 标题右键菜单 ≡ 溢出菜单 | ✅ 核销（09-04 静默口径） | 源码共享 `sessionMenuActions`（ChatAreaView L78/L96）+ 溢出 10 项逐字实测 `walk3/overflow_menu.txt`@22:00 + **09-04 用户池 B 拍板（认可 A 层在册证据）** | 无 |
...
| #6a tint 即时切换 | ✅ 核销（09-04 静默口径） | `ThemeLiveRenderTests`（蓝→橙主导序翻转+还原逐点复原，锁屏 0.07s）+ 02:26 xcresult 867 项在册 + **09-04 用户池 B 拍板** | 无 |
| #6b 社区包导入 | ✅ 核销（09-04 静默口径） | `FileThemePackageTests` 全套内核（spec/目录/校验/glass/边界/sanitizeID）+ 库级 e2e 门禁在跑 + **09-04 用户池 B 拍板** | 无 |
| #7 拖拽落位反馈 | ✅ 核销（09-04 静默口径；含「手动拖一次」子句 = ㉓ 第二分支·认可在册） | `SidebarDropHighlightRenderTests`（两级 0.14/0.06 规则+比例锁定）+ `onMoveSession` noChange 单测 + **09-04 用户池 B 拍板**（A-f 手动拖转 G3 可选观察项） | 无 |
| #8 减弱透明度降级 | ✅ 核销（09-04 静默口径） | `GlassSurfaceTests` 降级链（override=true 恒 solid / env 键直落 solid / resolveReduceTransparency 优先级矩阵 / 容器仅 native 包裹）在册 02:26 xcresult 867 项＋当日 pr 783 复跑 ✅（补记㉔①）+ `reduceTransparencyTestOverride` 测试缝 + **09-04 用户池 B 拍板** | 无 |

> 口径：✅=终版核销；◐=功能内核已锁（离屏/源码级），真机视觉帧为补证（不阻塞缺陷判定）；⏳=待解锁自动轮。R2 实况清单见 DB 盘点条目（空 18/保留 2）。 2026-09-04 收口注记：#3/#6a/#6b/#7/#8 之 ◐→✅ 全部来自池 B 静默裁决（用户认可 A 层进程内证据=满足真机视觉帧子句，㉔ 二选一之第二项落定），拍板原文见 DECISION_INDEX 拍板记录区。
...
| 3 | R1 走测③顶栏右键≡溢出 | ✅（09-04 静默核销） | 源码共享+溢出实测 + 09-04 池 B 拍板 | 无 |
| 4 | R1 走测④删除确认→取消 | ✅ | 补记① 双证 | 无 |
| 5 | R1 走测⑤设置 sheet 双关闭 | ✅ | 补记②（Esc 口径） | 无 |
| 6 | R1 ＋零同步残留 | ✅ | 补记③ | 无 |
| 7 | R1 走测⑥主题 tint 切换还原 | ✅（09-04 静默核销） | ThemeLiveRender/FileThemePackage 全套 + 09-04 池 B 拍板 | 无 |
| 8 | R1 走测⑦拖拽反馈（含"手动拖一次"子句） | ✅（09-04 静默核销，子句经㉓第二分支闭环） | DropHighlight 单测+noChange 单测 + 09-04 池 B 拍板 | 无 |
...
> 口径：**2026-09-04 收口轮已执行**——#3/#7/#8/#9 改走池 B 静默裁决（㉔ 二选一之「认可 A 层证据」）核销，本表+终版总表+QUALITY_REPORT 同日条目+DECISION_INDEX+G3 手册 §2 五处同步完毕（=「文档全核销」）。剩余非 ✅ 仅 14（功能冻结宣告归 G3 用户发起）——10（R2 数据处置）已于 09-05 执行闭环。
...
> 2026-09-04 G0 收口轮补记❸（**池 B + G1-SCOPE 拍板入册，R1 八项全 ✅，冻结 tag 就位**，接续❶）：① **用户拍板**：chat 答复「1」= 选定「认可池 B + G1-SCOPE」→ 按 ㉔ 二选一，#3/#6a/#6b/#7/#8 以「A 层进程内证据即满足真机视觉帧子句」核销；#7「用户手动拖一次」子句走 ㉓ 第二分支（认可在册）闭环，A-f 手动拖转为 G3 可选观察项而非验收前置。② **G1-SCOPE 认可**：双轴审计范围（轴1 A1–A19+材质提案 F3/F4；轴2 W1–W8 观察窗清单）无增删，DoD「范围经确认」条正式闭合。③ **文档同步**：终版总表+DoD 审计表状态列 ✅、DECISION_INDEX 拍板记录区两条、G3 手册 §2 池 B 表四行勾选。④ **冻结基线 tag** `g0-baseline-20260904` @本轮 HEAD；G0 余项仅 D-6/R2 处置（待拍板）与 §5 冻结宣告（归 G3）。⑤ 门禁：纯 docs-only 豁免（铁律 2），镜像+tag 推后 ls-remote 对账。编号 sanity：❸=U+2778 断言通过（教训㊺沿用）。

> 2026-09-04 D-10 执行轮补记❹（**(a) 透明窗底实施 @`ee1da43`**，接续❸）：① **拍板**：用户「1」= 下一步列表第 1 项 × 选项首位 → D-10=(a)（双位落位记录在总账，误读可单 revert 回退）。② **改动三处联动**：`HarnessApp.swift` makeWindow 经新增测试缝 `applyGlassSampling`（isOpaque=false + backgroundColor=.clear）；`ContentView.swift` 根 bgPrimary 实底只贴 mainContent；`SidebarView.swift` 列根加 `GlassSurface(.regular, cornerRadius:0)` 玻璃底（链位在 glassSurfaceContainer 之外=独立表面，不与 tab morph 面配对；native/legacy/solid 三级回落复用既有降级链组件）。③ **A 层证据**：新 `WindowGlassSamplingTests`（装配前默认态判别力断言 + 装配后 isOpaque=false/alpha=0，进程内零前台）；目标测试 20/20；build 0 警告；swiftformat→lint strict 0。④ **实况终裁**：透窗采样成败仅 G3 A-d 目检（㊹；ImageRenderer 不渲染 glass 的既知限制），手册 A-d/§4 行已改写为「实施效果判定」。⑤ **门禁**：pr @`ee1da43` 本会话后台跑（日志 /tmp/ci_r9_pr.log，rc 随行落账）；leaks/main/xcode 三门禁按 ㊻ 用户在场纪律顺延缺席窗口，DoD 行⑫暂沿用 @193d567 组并如实注记。⑥ **已知边界**：F5（sheet 自铺底，D-12 未拍）在透明窗底仍挡 sheet 区折射——已登记不擅动。
> 补记❹终态增注：pr 门禁 **rc=0 @58d7ab7** 全量绿（两连拦均设计内：A12 注册表首航拦截 + swiftformat consecutiveSpaces；教训两条已入 QUALITY）。当日 HEAD 链：ee1da43→1b4b303→f09184f→style 提交。

> 2026-09-04 D-5 拍板轮补记❺（**G4c 启动：D-5=做，设计入册**，接续❹）：① **拍板**：用户「1」= blocked#6 报告后列项首位 D-5 × 选项首位「做」（双位落位在总账，误读单 revert 可回退；代码未开跑回退成本最低）。② **材料立场如实留档**：层2 增量收益 ≈0（CENSUS 总表①②层1 已成立、③④结构性不适用），「做」= 用户裁决（社区插件拿来即用历史立场）而非材料推导。③ **设计入册** `docs/G4C_SIDECAR_DESIGN.md`：sidecar=MCP stdio server 复用 `StdioMCPClient`（Swift 零新协议）；默认策略 S-1 本机 Node 发现 / S-2 bridge 钉版本安装 / S-3 MCP stdio / S-4 逐包显式授权（均可否决）；安全红线按上游 README「沙箱非安全边界」原文对齐（独立进程+env 洗刷+「JS=任意代码执行」授权声明+网络不假拦截）；阶段 C0–C4 全 A 层验收（实包 ≥3 CLI 层）。④ **⚠️待查清单**三条入档（Cordis 宿主 API / apply(ctx) 工具枚举 / façade 最小集排序），实施首轮即实查上游源码+npm 实包，❌脑补。⑤ 门禁：docs-only 豁免；镜像+对账。
> 2026-09-04 G4c-C1 轮补记❻（**bridge 骨架 selftest+stdio 双绿，零前台**，接续❺）：① `tools/cordis-bridge/` 落位（钉版本安装 6s 实证）；② 契约四点运行时实证入册（DESIGN §7，含相对路径→file URL 教训）；③ selftest PASS（list+call）+ stdio 三帧冒烟全过（initialize 应答=在册 StdioMCPClient 期望形制）；④ 本批零 Swift 改动，门禁 Swift 面零影响如实注记；⑤ 镜像+对账。
> 2026-09-04 G4c-C2 轮补记❼（**dsh-crew 实包装载成功=「拿来即用」首例实锤**，接续❻）：① inject 普查 n=4 + 第二注册面 `ctx.web.registerSearchProvider` 实拆发现；② façade v2（reflect.provide 显式 stub+web 采集半+装载前自动铺桩）双 fixture selftest 全 PASS；③ dsh-crew rc7 tools/list 返回真实 describe_image/generate_image（call 面因外部副作用**不实测**，红线在册）；④ zai/hindsight 失败均=依赖供给/路径问题非语义问题，正式 add=npm install 流程定型入 §8/C4 入口；⑤ 本批 Swift 零改动；镜像+对账。
> 2026-09-04 G4c-C3 轮补记❽（**CLI 全链实锤：真实社区包经我方在册宿主客户端装载成功**，接续❼）：① `dsh cordis add/list/remove/probe` 落位（授权清单+consent 门+测试缝）；② env 洗刷走 env -i 白名单零改在册客户端；③ 运行时三课（peer 闭包/npm reify 清 extraneous/tmp-symlink 击穿主模块判定）全部实证修复入册 DESIGN §10；④ zai@0.2.0 add→probe 握手+工具可见、remove→probe 复拒（S-4 双向实测）；⑤ opt-in 冒烟测试正例+skip 判别力双验；⑥ pr 全量门禁已触发 @代码批。
> 2026-09-04 G4c-C4 轮补记❾（**层2 兼容矩阵四样本入册：2✅+2❌根因归类**，接续❽）：① zai/crew 正式 add→probe 全通（scoped、rc-pin、peer 闭包含 ERESOLVE 史包）；② hindsight=宿主未发布服务结构性不可用/manager=façade 语义边界，失败分类学贡献普查；③ call 口径=selftest 在册+真实包不实测红线；④ add-local 半补丁当场回滚不留半成品（复杂度评估后的止损决策入 QUALITY）；⑤ docs-only 批（矩阵三处入册）；镜像+对账。
> 2026-09-04 G4c collector 轮补记❿（**矩阵 3✅+2❌：第三真实包+真实 create_skill 收割**，接续❾）：① 通用 registrar 采集面落位（第三注册面 skills.registerProvider 实证）；② web 专用/通用并存回归（替换式重构风险被 selftest 当场拦截——测试判别力设计价值实证）；③ fixture 路径去 cwd 依赖；④ 本批零 Swift（bridge 单文件），门禁 Swift 面零影响注记；⑤ 镜像+对账。

> 2026-09-05 G4 DoD 核销轮补记⓫（**v8 DoD 的 G4 三格首次正式核销：逐格验表体不信标题，含不粉饰的双口径**，接续❿）：
> ① **核销依据（逐格亲验）**：**(a) 层1 实测**——CENSUS L172 表体亲数 3 行真握手（`dsh-crew` 6 工具/726ms、官方 `server-filesystem` 14 工具/6.6s、`server-everything` 13 工具/4.6s，含 serverInfo 版本与证据通道列），另有层1.5 App 真实链路 opt-in（`MCPCommunityAppFlowTests` 默认 skipped／flag 开 passed 0.096s，走用户粘贴命令同一代码路径）；**(b) D-5 拍板**——DECISION_INDEX 在册 ✅=做（09-04，附「用户裁决非材料推导」如实注记）；**(c) 层2 矩阵 ≥3 实跑**——CENSUS L200 表体 3✅（zai 1 工具／dsh-crew 3 工具／skills-manager 3 工具含真实 `create_skill`）＋ 2❌，且 **G3 手册 C-d 已独立 3/3 复现**（09-04 深夜：crew 751ms／fs 4.9s／everything 3.0s），证据强度高于单看矩阵自述。
> ② **不粉饰的双口径（G3 走查时必须向你明示）**：DoD 字面（「≥3 实跑样例」）**达成**；但作为你的原始诉求「社区插件拿来即用」**仅部分成立——5 样本 3 通 2 阻塞**。两个 ❌ 根因均为**宿主侧未发布服务／façade 语义边界**（生态侧限制，非我方可修；若要覆盖须 DSH 宿主发布对应服务，属另行立项）。⇒ 记为「DoD 达成 ∧ 诉求部分达成」，禁止把前者当后者向你申报。
> ③ **未落笔的错误亦入册（教训第三次）**：本轮曾据 `grep "G4"` 在 G3 手册零命中，判「手册缺兼容矩阵走查项」——实际手册 §3 的 **C-c／C-d 正是**该项（手册措辞是「插件兼容／兼容矩阵」）。差一步就写出「文档缺项」的不实论断，靠**读 §3 标题与表体**证伪。教训：**单一关键词零命中 ≠ 内容缺失**，必须先换同义表述与通读小节结构再判（本项目同类已三次：全/半角竖线锚点、`screencapture` 多写 `-x`、本次关键词漏检）。
> ④ 安全姿态复核（全样本一致，仍成立）：只 `initialize`+`tools/list`、**零 `tools/call`**；子进程环境最小化（PATH/HOME 白名单，HOME→沙箱）；超时必 kill；`--ignore-scripts` 阻断 postinstall。

> 2026-09-05 leaks 否证轮补记⓬（**第四门禁 rc=0 补齐 ＋「本机 task-inspection 通道 wedge」归因被对照实测推翻**，接续⓫）：
> ① **门禁事实（@c2b5ef0，A 层零前台零输入）**：`tools/ci-local.sh leaks` 整段 rc=0／28s ⇒ marker `2026-09-05 18:09:07 mode=leaks rc=0`；`leaks --atExit -- .build/debug/MemProbe 500` 报 `Process: 2460 nodes malloced for 485 KB / 0 leaks for 0 total leaked bytes`；外部 RSS 峰值护栏 20,928KB（阈值 524,288KB）；收尾 `pgrep -fl MemProbe` 零残留。四门 marker 自此齐备（pr@12:54:34／main@11:45:08／xcode@12:19:54／leaks@18:09:07）。
> ② **marker 与 HEAD 的适用性核验（不靠记忆）**：`git log --since=12:54:35 --name-only` 显示其后 8 枚提交仅涉 `tools/ci-local.sh` 与 docs，**无 `.swift`** ⇒ 另三门判据面未变，其 marker 可沿用；本行申报只把「leaks」从缺测变为实测。
> ③ **对照实验设计（本轮新增，三条通道互相独立）**：`sample <sleep 金丝雀> 1 -file …` rc=0/117 行；`sample 58301 1 -file …` rc=0/**1219 行**（对存活 2天5h 的 GUI 旧构建 attach 正常）；`leaks <金丝雀 pid>` rc=0/`0 leaks`；`leaks --atExit -- MemProbe 5` rc=0/1s。⇒ 三通道均可用，D-13 原「三条同时无返回」的通道级归因**撤回**。
> ④ **诚实边界（不把否证写成新归因）**：当时挂起确有在册证据（MemProbe 停 T 态、ps 显示 `TN`），但 `/tmp` 原始日志仅存今日文件 ⇒ 真实失败模式**不可反推**；另有一种未排除的解释须登记：`sample` **参数误用会瞬时 rc=255 并打印 usage**（本 session 我自己先踩一次：位置参数写文件名而非 `-file`），该现象易被误记为「无输出/无返回」。定论口径＝**间歇性故障、病因未定、当前不可复现**，不是「本机永久不可用」，也不构成任何需要你重启会话的依据。
> ⑤ **教训入档（归因类误判第四起 ＋ 对照设计缺陷一起）**：前序在册三起＝屏上窗 TCC 归因（`713f86b` 撤回）、CLI 建窗 SIGTRAP 归因、「App 未运行」存在性误判（`4b0686d` 自纠）。本轮另暴露**金丝雀对照无效化**：上一轮以 `sample` 对 sleep 有效推断「wedge 属目标进程」，但其被测侧命令本身语法错 ⇒ 该对照从未成立，其推论不得引用。固化约束：**归因前须「同命令 × 两类目标」对照，且先自证命令语法有效（usage/rc 先读）**；否则宁可写「病因未定」。
> ⑥ **口径后果（对你有用的部分）**：DoD「四门禁全绿」此刻为真（走查时 @最终HEAD 复验）；**你少出场一次**——D-13 的 (a) 重启登录会话与 (b) 降级口径均不再需要，拍板池由 9 项减为 8 项。门禁脚本内写死「本机 task-inspection 通道不可用／重启登录会话」的失败提示已改为中性处置顺序（复跑优先），避免继续误导后续轮次。

> 2026-09-05 层3 普查轮补记⓭（**回答「DSH 社区主题插件能不能拿来即用」：机制上不能，且证据在宿主侧**，接续⓬）：
> ① **为什么开这一层**：G4 矩阵此前只覆盖「标准 MCP server（层1）」与「宿主注入型 cordis 工具插件（层2）」，而社区还有一整类**主题/UI 插件**——它与本项目"主题全插件化"直接同名，是最容易被误当成"已覆盖"的一类。不能外推，只能实测。
> ② **样本（npm 实查，本机已解包闭包）**：`dsh-theme-kit@0.1.2` 与 `@guillaumemeyer/dsh-themes@0.1.1`，两者 `package.json` 均声明 `"dsh": { "client": { "platform": "web" } }`，client 入口是浏览器代码（theme-kit `lib/client.js` 浏览器 API 命中 186 **行**：`document.body`×15／`createElement`×9／`getElementById`×5；guillaumemeyer 11 行：`document.querySelector`／`document.head`／`createElement`）。
> ③ **决定性证据来自宿主而非样本包（铁律 7：上游 @47f9438 源码/官方文档）**：官方 `docs/subsystems/client-modules.md` L49 逐字 "declaring `dsh.client` (**`platform: 'web'`**…)"；装载发现脚本 `scripts/dev-web.ts` L42 逐字 `if (manifest.dsh?.client?.platform === 'web') dirs.push(…)`（**严格等于**）；全仓取值普查 `packages/**/package.json` 内 `"platform"` **39 处全部 = `"web"`，零例外**（含上游自家 registry／ui-cordis／api-gateway）。
> ④ **结论与分类学新增**：DSH「client 插件」机制只有 web 一个平台取值 ⇒ **社区主题/UI 类插件在原生 macOS 客户端结构性不可用**，登记为失败分类学**第③类：宿主平台枚举不含我方平台**（①宿主未发布服务／②façade 未实现真实语义／③本类）。非我方桥或宿主缺陷，也与既有条款「明确不做：Cordis UI 注入界面兼容」和基线「纯原生 UI／❌WebView」三处一致 ⇒ **不新增拍板项**，只补走查须知。
> ⑤ **计数口径防注水**：层2 仍 5 样本 3✅+2❌（未动）；层3 的 2 样本记「结构性不适用」，**不计入层2 通过率**；合计普查社区包 **9 个**（层1 3＋层2 5 去重 7，＋层3 新 2）。落点：CENSUS 层3、拍板卡③、G3 手册 C-c 走查须知。
> ⑥ 本轮两处**写台账时的自纠**（详见 QUALITY 同日条）：`grep -c` 计的是命中**行**数而非命中次数，措辞已由"处"改"行"；合计包数初写 7 漏了层3 新增 2，实为 9——凡"合计/去重"类数字必须现算现写。

> **2026-09-05（静默轮 7 枚提交总述 · 报告链补记 · §18/§19/§20/§21 四节新增）**：本轮全程 A 层静默（零前台/零键鼠/未触 `.swift`），提交链 `4431fb9`→`5947e65`→`3e8b55a`→`5d0ebae`→`18a85b3`→`0554c65`→`f58e81b`，每枚均推镜像并 ls-remote 实测 MATCH；四门禁沿用口径由 `3e8b55a` 起的**子树指纹机械证明**背书（Apps `6ba1b52c6197`／Packages `1c495203e2a1` 与 @e5c6c37 逐位相同）。**四件改变后续判断的产出**：① **推翻 §18.5 误判**——官方在线教程文章实测含「nearest edge ≤ container's spacing」逐字原文，避免一次把正确注释改错的返工；② **§19 补录三句缺录原文**并据 at-rest 融合告警（我方 tab 容器 179pt vs 内部 6pt＝29.8×）**新立 D-15**（A2 与 D-1 解耦，收敛 spacing 至 52 ＋ 远距显式 `.materialize`，标推荐）；③ **§20 把「官方不存在 X」变成有退出码的检索门**（三件工具入仓：`md-table-lint.py`／`decision-pending.sh`／`apple-quote-index.sh`，全部实测失败路径退出码）；④ **§21 代码注释「官方」引用全量审计**（54 行三分口径闭合）＝新抓 B 档 10 条＋C 档 1 条，跨领域 4 条断言核验全部准确。**待 idle 执行登记（不占拍板位）**：§21.2 的 11 条 `.swift` 注释修正＝G2 首轮附带项，命令序列：改注释 → `bash tools/ci-local.sh pr`（须 `HIDIdleTime ≥ 60min`）→ 通过后小提交 → 推镜像 → 本表与 QUALITY 入册；若同轮顺带实施 D-15(b)（一行 spacing 取值＋一个 transition 修饰符）则一并跑第四门禁 leaks。**待你出场**：拍板 **10 项**（现算口径 `tools/qa/decision-pending.sh`）＋ Codex 截图（§16 W1–W8）＋ G3 走查（P6 仅剩手跑重启一步）＋ D-14。当前 HEAD `f58e81b`。