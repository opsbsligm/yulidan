...
> 2026-09-04 G4c-C1 轮补记❻（**bridge 骨架 selftest+stdio 双绿，零前台**，接续❺）：①  落位（钉版本安装 6s 实证）；② 契约四点运行时实证入册（DESIGN §7，含相对路径→file URL 教训）；③ selftest PASS（list+call）+ stdio 三帧冒烟全过（initialize 应答=在册 StdioMCPClient 期望形制）；④ 本批零 Swift 改动，门禁 Swift 面零影响如实注记；⑤ 镜像+对账。
