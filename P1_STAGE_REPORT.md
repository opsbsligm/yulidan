...
| 12 | 四门禁全绿 @当前HEAD | **⚠️ 3/4（09-05 诚实降级）** | `pr` rc=0@12:54:34／`main` rc=0@11:45:08／`xcode` rc=0@12:19:54（done 标记＋日志可核）／**`leaks` 无 rc=0 marker** | **leaks 一门未过**：`leaks`/`lldb`/`sample` 三条 task-inspection 通道本机 wedge（金丝雀二次探测仍未自愈），属工具通道退化非代码回归；09-04 21:20 该门曾 rc=0。恢复条件＝D-13 拍板（重启登录会话后复跑原判据／或你认可 RSS 峰值护栏为临时替代）。⇒ DoD「四门禁全绿」在 leaks 恢复前**不得申报达成** |
