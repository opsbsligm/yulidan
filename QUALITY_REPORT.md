...
| 单元测试 | ✅ 927/927 | Swift Testing 705（152 suites）+ XCTest 222/222，0 失败（周末覆盖审计轮 5 +7：MCP 包 manager 非 stdio/stdio 诊断/服务发现兜底新文件 MCPGapCoverageTests；前轮 +15；日志 /tmp/ci_pr_agent5mcp.log） |
| 本地 CI 模拟 | ✅ 四门禁全绿（周末覆盖审计轮 5 复跑 @e951e0e） | `tools/ci-local.sh`：pr /tmp/ci_pr_agent5mcp.log（705/705，152 suites + XCTest 222/222，合计 927）+ main /tmp/ci_main_agent5mcp.log（Release + 全量 + 覆盖率：核心 14 包 96.70%（6883/7118）均 ≥90% / 18 包全量 94.81%（9448/9965）/ MCP 96.00%（32 未覆盖行））+ leaks /tmp/ci_leaks_agent5mcp.log（MemProbe 500 → **0 leaks**）+ xcode /tmp/ci_xcode_agent5mcp.log（**TEST SUCCEEDED**）；前轮基线：920/920 四门禁 @c871b1b 轮（/tmp/ci_{pr,main,leaks,xcode}_agent5.log）；GitHub 远端激活前四门禁以 ci-local 为准 |
...
| 测试代码（87 文件） | 19,823 行（周末覆盖率加固 +38 用例 / 覆盖审计轮 2 +6 / 轮 3 +6 / 轮 4 +15 / 轮 5 +7，共 +136 用例 +1,685） |
...
| 提交总数 | 186（周末覆盖审计轮 5：test `e951e0e` + 本轮 docs；前轮 184 @`9a65f8d`） |
