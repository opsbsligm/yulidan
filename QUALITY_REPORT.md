...
> 生成时间: 2026-08-20 04:20
> 项目版本: v0.3.4（后端 8 模块闭环 + 跨模块场景 4 项 + App 层场景 16 项 + 前端阶段 1/2/3a/3b/3c/3d 完成，HEAD `a0cb8ab`）
> 说明: 本轮完成 F6 WWDC26 Liquid Glass 表面系统（GlassSurface：三级降级链 solid→native→legacy + 原生 glassEffect + 减弱透明度无障碍 + 4 项单测 + 核心表面应用含 CardModifier 全局升级）；修复本 SDK ViewModifier @MainActor 隔离导致的 26 处 #ActorIsolatedCall 警告（零警告基线恢复）；覆盖率按 655 用例 run 以 llvm-cov export lcov 口径重测；门禁结果均为当前 HEAD 实测。
...
| 单元测试 | ✅ 655/655 | XCTest 180 + Swift Testing 475（92 suites），0 失败（含 4 项跨模块场景 + 16 项 App 层场景端到端 + 4 项 GlassSurface 表面系统） |
...
## 三、测试用例与行覆盖率（2026-08-20 04:05 全量重测，llvm-cov export lcov 口径）

| 模块 | 行覆盖（仅源文件，655 用例 run 实测） | 状态 |
|------|--------|------|
...
| HarnessApp（UI 层） | 24.3%（1225/5037） | 说明：SwiftUI 视图层，不计入 90% 核心基线（ViewModel 逻辑已由 HarnessAppTests 覆盖）。较上版 23.9%（1183/4957）+42 命中行（同口径：llvm-cov lcov 插桩行），由 GlassSurface 新文件 + 4 项表面系统单测驱动 |

**总计: 655 个测试用例（XCTest 180 + Swift Testing 475），全部通过。**
**16 个后端包（含 WebUI）行覆盖 94.2%（6701/7123）；不含 WebUI 15 包 94.2%（6202/6581）；14 个核心包（除 Notifications）均 ≥90%。**（勘误：上版「15 包 6701/7123」实际含 WebUI，本版起拆分标注）
...
| 源码（Apps/HarnessApp，22 文件） | 6,284 行（+149：GlassSurface 156 − CardModifier/5 视图接入净 7） |
| 源码（Apps 辅助 target：DSHCLI/HarnessCore/HarnessPluginWorker/MemProbe，10 文件） | 1,250 行 |
| 源码合计（109 文件） | 19,257 行 |
| 测试代码（Packages 10,094 + Apps 2,499） | 12,593 行（+44：GlassSurfaceTests 4 场景） |
...
已完成（本轮）：F6 WWDC26 Liquid Glass 表面系统闭环（`a0cb8ab`：GlassSurface 三级降级链 + 原生 glassEffect + 无障碍降级 + 4 项单测 + 核心表面应用）；26 处 #ActorIsolatedCall 警告修复（零警告基线恢复）；655 用例基线 + 覆盖率 lcov 口径重测 + 15/16 包口径勘误。

1. **下一步（F7）：Codex 布局/交互对齐差距盘点** — 对标 Codex 界面范式逐项盘点主窗口布局、功能按钮/面板布局差距（用户 5 项 UI 要求中「布局对标 Codex」「按钮面板对齐」两项），盘点出差距清单后按价值排序实施
2. 视觉验收：GlassSurface（尤其 CardModifier 全局升级，影响所有 harnessCard 卡片）需用户实机确认（无头环境结构性限制）
