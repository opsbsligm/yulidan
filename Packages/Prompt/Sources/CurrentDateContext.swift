...
/// 位于 Prompt 包（共享）：App 主聊天/子 Agent 与 DSHCLI 无头路径统一接入
///
...
public enum CurrentDateContext {
    /// 中文星期名，下标 = Calendar.component(.weekday) - 1（1=周日）
    public static let weekdayNames = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"]

    /// 注入行前缀（幂等剥离用）
    public static let linePrefix = "当前时间："

    /// 将 `now` 格式化为「当前时间：2026-08-24（星期一）」
    public static func line(for now: Date, calendar: Calendar = .current) -> String {
    ...
    public static func inject(into systemPrompt: String, now: Date = Date(), calendar: Calendar = .current) -> String {
