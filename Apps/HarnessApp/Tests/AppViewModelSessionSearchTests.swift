...
struct AppViewModelSessionSearchTests {
    ...
    func searchFlow() async throws {
        ...
        let vm = AppViewModel()
        guard try await waitForInitialLoad(vm, s1) else {
        ...
        var results = try await waitForResults(vm)
        #expect(results != nil)
        ...
        results = await vm.searchResults
    ...
    func noMatch() async throws {
        ...
        let results = try await waitForResults(vm)
